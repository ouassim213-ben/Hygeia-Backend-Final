...
function getStringWidth(text, styles, doc) {
	...
	return (Array.isArray(text) ? text : [text]).map(function(text) {
...
function parseHtml(doc, input, window, includeHiddenHtml, useCss) {
	...
	var tableElement;
	if (typeof input === "string") tableElement = window.document.querySelector(input);
...
function parseRowContent(supportedFonts, scaleFactor, window, row, includeHidden, useCss) {
	...
	for (var i = 0; i < row.cells.length; i++) {
		...
		if (includeHidden || style_1.display !== "none") {
			...
			if (useCss) cellStyles = parseCss(supportedFonts, cell, scaleFactor, style_1, window);
			resultRow.push({
...
function createColumns(columns) {
	return columns.map(function(input, index) {
		var _a;
		var key;
		if (typeof input === "object") key = (_a = input.dataKey) !== null && _a !== void 0 ? _a : index;
		else key = index;
...
function drawCellRect(doc, cell, cursor) {
	...
	if (typeof cellStyles.lineWidth === "number") {
		...
		if (cellStyles.fillColor) doc.rect(cell.x, cursor.y, cell.width, cell.height, "F");
...
function applyPlugin(jsPDF) {
	...
	jsPDF.API.autoTableHtmlToJson = function(tableElem, includeHiddenElements) {
		...
		return {
			columns: ((_a = head[0]) === null || _a === void 0 ? void 0 : _a.map(function(c) {
				return c.content;
			})) || [],
			rows: body,
			data: body
